...
public class ChapActivity extends AppCompatActivity {
    ...
    public static boolean netCheck(Activity activity) {
        ...
        try {
            ...
            if (!stat) {
                ...
                HelloApp.context.startActivity(noCon);
                activity.finish();
                Log.d("NET_STAT", "NO SERVER CON");
                return false;
            } else {
                Log.d("NET_STAT", "SERVER CON");
                return true;
            }

        } catch (IOException e) {
            Log.d("NET_STAT", e.toString());
        }
    ...
    private static boolean executeReq(URL urlObject) throws IOException {
