...
class PodcastPlayer : AppCompatActivity(), MediaPlayer.OnPreparedListener {
    private lateinit var bebas: Typeface
...
//        pg.cancel()
        ...
        var time = if(h != 0) {if(h<10) "0$h:" else "$h:"} else{ "" }
        time +=if(m<10) "0$m:" else "$m:"
        time +=if(s<10) "0$s" else "$s"

        ppFullTime.setText(time)
